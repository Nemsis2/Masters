...
"""
validates a model by testing its performance by assessing coughs on the corresponding validation fold.
"""
def validate_model_coughs(model, train_outer_fold, train_inner_fold, interpolate, batch_size):
      val_data, val_labels, val_names = extract_val_data(K_FOLD_PATH + MELSPEC, train_outer_fold, train_inner_fold)

      # preprocess data and get batches
      val_data, val_labels, val_names, lengths = create_test_batches(val_data, val_labels, val_names, interpolate, batch_size)

      # test model
      results = test(val_data, model, lengths)
      
      # assess the accuracy of the model
      auc, sens, spec  = cough_performance_assess(val_labels, results)
      ...
      del val_data, val_labels, val_names
      gc.collect()
...
def validate_model_patients(model, train_outer_fold, train_inner_fold, interpolate, batch_size):
      data, labels, names = extract_val_data(K_FOLD_PATH + MELSPEC, train_outer_fold, train_inner_fold)
      ...
      threshold = get_optimal_threshold(labels, results)
